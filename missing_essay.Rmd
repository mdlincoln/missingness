...
opts_chunk$set(cache = TRUE, echo = FALSE, sanitize = TRUE, warning = FALSE, message = FALSE, fig.align = "center")
...
```{r plot_diversity, fig.show = "animate"}
library(gganimate)
animation::ani.options(interval = .2)

boot_anim <- ggplot(diversities, aes(x = window_point, y = div, frame = boot_iteration)) +
  geom_line(aes(cumulative = TRUE, group = boot_iteration), alpha = 0.1) +
  geom_line(aes(frame = boot_iteration), color = "red", size = 1)

gg_animate(boot_anim)
```
